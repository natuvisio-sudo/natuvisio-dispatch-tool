...
# --- 2. CSS & STYLING (MATCHING ADMIN HQ) ---
def load_css():
    css = """
    <style>
...
def check_password():
    ...
    with col2:
        if st.button("Access Partner Portal", use_container_width=True):
            if password == passwords.get(brand):
...
def partner_interface():
    ...
    if not df.empty:
        ...
        brand_orders = pd.DataFrame(columns=df.columns)
    ...
    with tab1:
        ...
        if not pending.empty:
            ...
            for idx, row in pending.iterrows():
                # Locate original index in main DF to update correctly
                original_idx = df[df['order_id'] == row['order_id']].index[0]
                ...
                with st.expander(f"🔔 {row['order_id']} - {row['sku']} × {row['quantity']}", expanded=True):
                    ...
                    with col_btn1:
                        if st.button(f"📦 Start Processing", key=f"process_{row['order_id']}"):
                            df.loc[original_idx, 'status'] = 'Processing'
                            save_dispatch_history(df)
                            st.success("Order moved to processing!")
                            st.rerun()
                    
                    with col_btn2:
                        # Direct Ship from Pending
                        pass
        else:
    ...
    with tab2:
        ...
        if not processing.empty:
            ...
            for idx, row in processing.iterrows():
                ...
                # Ship action
                with st.expander(f"Ship Order {row['order_id']}", expanded=True):
                    tracking = st.text_input(
                        "Enter Tracking Number",
                        key=f"track_process_{row['order_id']}"
                    )
                    
                    if st.button(f"Confirm Shipment", key=f"ship_process_{row['order_id']}"):
                        if tracking:
                            df.loc[original_idx, 'status'] = 'Shipped'
                            df.loc[original_idx, 'tracking_number'] = tracking
                            save_dispatch_history(df)
