...
def load_css():
    ...
    <style>
        ...
        .status-alert-yellow {{ border-left: 4px solid #F59E0B !important; }}
...
def get_icon(name, color="#ffffff"):
    icons = {
        ...
        "doc": f'<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>'
    }
...
def admin_dashboard():
    ...
    # --- TAB 1: NEW DISPATCH (Cart Builder) ---
    with tabs[0]:
        cL, cR = st.columns([1.6, 1])
        with cL:
            st.markdown('<div class="glass-card"><h4>📝 Order Entry</h4>', unsafe_allow_html=True)
            if 'cart' not in st.session_state: st.session_state.cart = []
    ...
    # --- TAB 2: NEW ORDERS (Pending Notification) ---
    with tabs[1]:
        st.markdown("### 🔴 Pending Notifications")
        ops_df = get_db(CSV_DISPATCH, cols_dispatch)
        pending_df = ops_df[ops_df['Notified'] == 'NO'].sort_values("Time", ascending=False)
        
        if not pending_df.empty:
            for idx, row in pending_df.iterrows():
                with st.container():
                    st.markdown(f"""
                    <div class="glass-card status-alert-red">
                    ...
                    with c1:
                        if st.button("Mark Notified", key=f"ntf_{idx}"):
                            # Update main DF
                            main_idx = ops_df[ops_df['Order_ID'] == row['Order_ID']].index[0]
                            ops_df.at[main_idx, 'Notified'] = "YES"
                            ops_df.at[main_idx, 'Status'] = "Brand Notified"
                    ...
                    with c2:
                        ...
                        st.markdown(f'<a href="{link}" target="_blank"><button style="width:100%;background:#25D366;border:none;padding:10px;border-radius:5px;color:white;">Open WhatsApp</button></a>', unsafe_allow_html=True)
                    st.markdown("</div>", unsafe_allow_html=True)
        else:
            st.info("No new orders pending notification.")

    # --- TAB 3: PROCESSING (Active Logistics) ---
    with tabs[2]:
        st.markdown("### ✅ Active Logistics")
        ops_df = get_db(CSV_DISPATCH, cols_dispatch)
        active_df = ops_df[ops_df['Status'].isin(['Brand Notified', 'Dispatched'])].sort_values("Time", ascending=False)
        
        if not active_df.empty:
            for idx, row in active_df.iterrows():
                glow = "status-alert-yellow" if row['Status'] == 'Brand Notified' else "status-alert-green"
                st.markdown(f"""<div class="glass-card {glow}"><h4>{row['Order_ID']} | {row['Status']}</h4><small>{row['Items']}</small>""", unsafe_allow_html=True)
                ...
                with c1:
                    if row['Status'] == 'Brand Notified':
                        ...
                        if st.button("Confirm Dispatch", key=f"dsp_{idx}"):
                            main_idx = ops_df[ops_df['Order_ID'] == row['Order_ID']].index[0]
                            ops_df.at[main_idx, 'Tracking_Num'] = trk
                            ops_df.at[main_idx, 'Status'] = "Dispatched"
                            update_db(CSV_DISPATCH, ops_df)
                            st.rerun()
                with c2:
                    if row['Status'] == 'Dispatched':
                        ...
                        if st.button("Mark Delivered", key=f"dlv_{idx}"):
                            main_idx = ops_df[ops_df['Order_ID'] == row['Order_ID']].index[0]
                            ops_df.at[main_idx, 'Status'] = "Completed"
                            update_db(CSV_DISPATCH, ops_df)
                            st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
        else:
    ...
    with tabs[3]:
        ...
        ops_df = get_db(CSV_DISPATCH, cols_dispatch)
        if not ops_df.empty:
            st.dataframe(ops_df.sort_values("Time", ascending=False), use_container_width=True)
        else:
            st.info("No history.")

    # --- TAB 5: FINANCIALS (Invoicing & Payouts) ---
    with tabs[4]:
        sub_tabs = st.tabs(["🧾 Commission Invoicing", "🏦 Vendor Payouts"])
        
        # SUB-TAB 1: COMMISSION INVOICING
        with sub_tabs[0]:
            st.markdown("#### Partner Muhasebe (Fatura)")
            fin_df = get_db(CSV_FINANCE, cols_finance)
            pending = fin_df[fin_df['Invoice_Ref'].isna() | (fin_df['Invoice_Ref'] == "")]
        ...
        with sub_tabs[1]:
            ...
            with c1:
                ...
                p_brand = st.selectbox("Select Vendor", list(BRAND_CONTRACTS.keys()))
                ...
                amt = st.number_input("Payment Amount", 0.0, float(bal) if bal > 0 else 0.0)
    ...
    with tabs[5]:
        ...
        with d1:
            ...
            st.download_button("📥 Dispatch Logs", convert_df(df), "dispatch_logs.csv", "text/csv")
            
        with d2:
            df = get_db(CSV_FINANCE, cols_finance)
            st.download_button("📥 Financial Ledger", convert_df(df), "financial_ledger.csv", "text/csv")
            
        with d3:
            df = get_db(CSV_INVOICES, cols_invoices)
            st.download_button("📥 Invoices", convert_df(df), "invoices.csv", "text/csv")
            
        with d4:
            df = get_db(CSV_PAYOUTS, cols_payouts)
            st.download_button("📥 Payouts", convert_df(df), "payouts.csv", "text/csv")
    ...
    with tabs[7]:
        ...
        # Simple timeline of all orders sorted by time
        df = get_db(CSV_DISPATCH, cols_dispatch)
        if not df.empty:
            log_view = df[['Time', 'Order_ID', 'Brand', 'Status', 'User_Action_Log' if 'User_Action_Log' in df.columns else 'Status']].sort_values('Time', ascending=False)
            st.dataframe(log_view, use_container_width=True)
        else:
            st.info("System log empty.")
...
def partner_dashboard():
    ...
    tabs = st.tabs(["📋 PENDING", "📦 IN PROCESS", "✅ COMPLETED", "💰 FINANCIALS"])
    ...
    # Financials Tab for Partner
    with tabs[3]:
        st.markdown("### 💰 Financials (Muhasebe & Ödemeler)")
        
        # Invoices Sent to Brand
        st.markdown("#### Commission Invoices (Faturalar)")
        inv_df = get_db(CSV_INVOICES, cols_invoices)
        my_inv = inv_df[inv_df['Brand'] == brand]
        if not my_inv.empty:
            st.dataframe(my_inv[['Invoice_Ref', 'Date', 'Total_Due', 'Paid_Status']], use_container_width=True)
        else:
            st.info("No invoices received.")

        # Payouts Received
        st.markdown("#### Payouts Received (Ödemeler)")
        pay_df = get_db(CSV_PAYOUTS, cols_payouts)
        my_pay = pay_df[pay_df['Brand'] == brand]
        if not my_pay.empty:
            st.dataframe(my_pay, use_container_width=True)
        else:
            st.info("No payouts received.")
